# Top of Mind 2026 — Brasília

Apresentação HTML interativa dos resultados da pesquisa Top of Mind 2026, do Jornal de Brasília, conduzida pela **Opinião Inteligência de Mercado**.

## Como usar

- **Apresentação**: [`apresentacao.html`](apresentacao.html) — single-page deck navegável (89 slides 16:9). Use as setas do teclado (← →) ou os controles no rodapé.
- **Relatório completo (PDF)**: o botão **Relatório Completo** na capa baixa o arquivo `Top of Mind 2026 — Relatorio.pdf`.
- **Relatórios por categoria**: 72 arquivos HTML em [`categorias/`](categorias/). O botão **Gerar relatório** em cada slide de categoria abre um modal com opções de imprimir, salvar como PDF ou compartilhar por WhatsApp.

## Estrutura do repositório

```
.
├── apresentacao.html                       # apresentação principal
├── Top of Mind 2026 — Relatorio.pdf        # PDF da apresentação completa
├── categorias/                             # 72 relatórios A4 por categoria
│   ├── Q_10_Top_das_Marcas.html
│   ├── Q_11_Banco.html
│   ├── ...
│   └── Q_81_Cultura_Lazer_Esporte.html
└── assets/                                 # logos
    ├── logo_opiniao.png
    ├── logo_tom2026.png
    ├── logo_jb.webp
    └── logo_jb_favicon.jpg
```

## Sobre a pesquisa

- **Período de coleta**: maio de 2026
- **Amostra**: 726 entrevistas com população de Brasília (16+ anos)
- **Margem de erro**: ± 3,6 p.p. (IC 95 %)
- **Cobertura**: 26 das 33 Regiões Administrativas
- **Abrangência**: 12 segmentos · 72 categorias

## Contato

**Opinião Inteligência de Mercado**
[www.opiniao.inf.br](https://www.opiniao.inf.br) · 61 3212-3000 · contato@opiniao.inf.br
